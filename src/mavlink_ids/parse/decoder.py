...
def decode_stream(conn: Any, *, blocking: bool = True) -> Iterator[Event]:
    ...
    `blocking` controls what happens when no message is ready:
      * True  (live link): wait for the next packet, so the stream continues
        indefinitely until the link closes.
      * False (replay file): a "nothing to read" means end-of-file, so we stop.

    This is a generator: it produces events lazily, one per loop, instead of
    building a giant list in memory.
    """
    while True:
        msg = conn.recv_match(blocking=blocking)
        if msg is None:
            break  # link closed (blocking) or end of the replay file (non-blocking)
